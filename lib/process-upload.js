...
function extractISBN(text) {
  var
    reg = /ISBN(-1(?:(0)|3))?(\s)*(\(pbk\))?:?\x20(\s)*[0-9]+[- ][0-9]+[- ][0-9]+[- ][0-9]*[- ]*[xX0-9]/g,
    isbn,
  ...
  if (matches) {
    ...
    isbn = matches[0].replace(/ISBN/g, '').replace(/\(pbk\)/g, '').replace(/:/g, '').replace(/ /g, '').replace(/-/g, '').trim();
  }
  return isbn;
}
/**
  ...
  async.series([
      ...
      function findUpload(callback) {
        ...
        models.Upload.findOneAndRemove({
          ...
          if (!result) {
            return callback(new Error('INVALID_IDENTIFIER'));
          }
          fs.exists(result.path, function(exist) {
            if (!exist) {
              return callback(new Error('FILE_NOT_EXIST'));
            }
