...
function mapRoutes(app) {
  ...
  app.post('/api/upload', authenticated, multipartMiddleware, function(req, res) {
    flow.post(req, function(status, filename, original_filename, identifier, type, path, totalSize) {
      // console.log('POST', status, original_filename, identifier, type, path, totalSize);
      if (status === 'done') {
  ...
  app.get('/api/upload/:identifier', authenticated, function(req, res) {
    ...
    models.Upload.find({
      ...
      if (err) {
        ...
        pdfLoader.loadPDF(upload, function(err, data) {
          if (!err) {
            debugger;
            var reg = /ISBN(-1(?:(0)|3))?:?\x20(\s)*[0-9]+[- ][0-9]+[- ][0-9]+[- ][0-9]*[- ]*[xX0-9]/g;
            console.log(reg.exec(data.toString()));
          } else {
            console.log(err);
          }
        });
