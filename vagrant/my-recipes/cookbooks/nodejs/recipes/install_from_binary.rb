...
#end
# node web site is too unreliable, especially from eu-west-1 region.  Use S3 instead.
include_recipe "fluidaws"
fluidaws_download_from_s3 "Download Node Binaries from S3" do
  aws_access_key_id node['aws_access_key_id']
  aws_secret_access_key node['aws_secret_access_key']
  bucket node['fr_s3_resource_bucket']
  key "packages/#{nodejs_tar}"
  target_location "/usr/local/src/#{nodejs_tar}"
  not_if { File.exists?("/usr/local/src/#{nodejs_tar}") }
end
